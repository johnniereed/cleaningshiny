...
// Progressive reveal enhancement. Content is visible by default so a script or cache issue can never hide the site.
...
const menu=document.querySelector('.mobile-drawer-trigger');
...
function openMobileDrawer(){
  if(!mobileDrawer||window.matchMedia('(min-width: 901px)').matches)return;
  mobileDrawer.hidden=false;
  mobileDrawer.classList.add('is-open');
  document.body.classList.add('mobile-drawer-open');
  menu?.setAttribute('aria-expanded','true');
}
menu?.addEventListener('click',e=>{e.stopPropagation();mobileDrawer?.classList.contains('is-open')?closeMobileDrawer():openMobileDrawer()});
mobileDrawerClose?.addEventListener('click',closeMobileDrawer);
mobileDrawer?.querySelectorAll('a').forEach(a=>a.addEventListener('click',closeMobileDrawer));
document.addEventListener('click',e=>{if(mobileDrawer?.classList.contains('is-open')&&!mobileDrawer.contains(e.target)&&!menu?.contains(e.target))closeMobileDrawer()});
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeMobileDrawer()});
window.addEventListener('resize',()=>{if(window.matchMedia('(min-width: 901px)').matches)closeMobileDrawer()});
...
async function initQuoteTurnstile(){
  ...
  try{
    ...
    turnstileWidgetId=window.turnstile.render(mount,{sitekey:config.siteKey,theme:'light'});
  }catch(err){
...
function startCleaningAutoplay(){
  clearInterval(cleaningAutoTimer);
  if(window.matchMedia('(max-width: 600px)').matches&&!window.matchMedia('(prefers-reduced-motion: reduce)').matches&&cleaningSlides.length>1){
    cleaningAutoTimer=setInterval(()=>showCleaningSlide(cleaningIndex+1),5000);
  }
}
...
function startFacilityAutoplay(){
  clearInterval(facilityAutoTimer);
  if(window.matchMedia('(max-width: 600px)').matches&&!window.matchMedia('(prefers-reduced-motion: reduce)').matches&&facilitySlides.length>1){
    facilityAutoTimer=setInterval(()=>showFacilitySlide(facilityIndex+1),5000);
  }
}
