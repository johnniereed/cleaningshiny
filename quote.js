...
async function verifyTurnstile(request,env,token){
  ...
  if(!result.success){
    ...
    console.warn('Turnstile Siteverify failure', {codes, hostname:result.hostname||'', action:result.action||''});
    return {success:false,reason:'failed',codes};
  }
...
export async function onRequestPost({request,env}){
  try{
    ...
    if(!turnstile.success){
      console.warn('Turnstile rejected quote request',turnstile.reason,turnstile.codes||[]);
